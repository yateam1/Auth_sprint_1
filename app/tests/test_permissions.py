...
def test_create_role(test_app, test_db, user_headers):
...
def test_create_role_without_required_fields(test_app, test_db, user_headers):
    client = test_app.test_client()
    resp = client.post(
        '/permissions/roles',
        data=json.dumps({}),
...
def test_create_role_with_same_name(test_app, test_db, user_headers):
    client = test_app.test_client()
...
def test_get_correct_role(test_app, test_db, user_headers):
    role = RoleFactory()
    client = test_app.test_client()
    resp = client.get(
...
def test_get_incorrect_role(test_app, test_db, user_headers):
    role_id = uuid4()
    client = test_app.test_client()
    resp = client.get(f'permissions/roles/{role_id}', content_type='application/json', headers=user_headers)
    data = json.loads(resp.data.decode())
    assert resp.status_code == 404
    assert f'Роли {role_id} не существует.' == data['message']


def test_patch_role(test_app, test_db, user_headers):
    role = RoleFactory()
    client = test_app.test_client()
...
def test_patch_incorrect_role(test_app, test_db, postfix, user_headers):
    role_id = uuid4()
    client = test_app.test_client()
    resp = client.patch(
...
def test_patch_role_same_name(test_app, test_db, user_headers):
    role = RoleFactory()
    client = test_app.test_client()
    resp = client.patch(
        f'/permissions/roles/{role.id}',
...
def test_patch_role_add_and_remove_users(test_app, test_db, user_headers):
...
def test_get_list_roles(test_app, test_db, count, user_headers):
    RoleFactory.create_batch(count)
    client = test_app.test_client()
    resp = client.get(f'permissions/roles', content_type='application/json', headers=user_headers)
    data = json.loads(resp.data.decode())
    assert resp.status_code == 200
